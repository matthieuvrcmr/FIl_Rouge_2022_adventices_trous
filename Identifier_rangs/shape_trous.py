...
def distinguer_trous_circularite_area(path_to_img, circularite_min=0.5, area_min=10, area_max=30):
    ...
    cv2.imwrite(image_path_without_ext+'_Circularité_aire_binaire'+ extension,th)
    ...
    cv2.imwrite(image_path_without_ext+'_Circularité_Aire'+ extension,image)
...
def distinguer_trous_area(path_to_img, area_min=10, area_max=30):
    ...
    th=255-th # inverser la couleur, car cv2.HoughCircles detecte les pixels blanc
    # cv2.imwrite(image_path_without_ext+'_Aire_binaire'+ extension,th)
    ...
    image = cv2.imread(path_to_img)
    cv2.drawContours(image,contours_trous,-1,(0,255,0),1)
    cv2.imwrite(image_path_without_ext+'_Aire'+ extension,image)
...
path_to_img = '/Comptage_trous/DJI_0009_trou.jpg'
...
print('circulatite + aire :', nb_trous)
